...
public class MockInitialContextFactory extends DefaultHandler implements InitialContextFactory {
    ...
    public Context getInitialContext(final Hashtable<?, ?> environment) throws NamingException {
        ...
        try {
            ...
            if (inputStream != null) {
                try {
                    return loadInitialContext(inputStream, environment, Thread.currentThread().getContextClassLoader());
                } finally {
                    inputStream.close();
                }
            }
    ...
    private Context loadInitialContext(final InputStream inputStream,
        ...
        parser.parse(inputStream, new ConfigParserHandler(initialContext, nameParser, classLoader));
